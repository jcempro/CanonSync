...
from CanonSync.src.core.cleanup import destination_cleanup
from CanonSync.src.core.file_operations import (
    apply_root_hidden_attribute,
    origin_to_destination,
    recursive_directory_iteration,
)
from CanonSync.src.core.retry import retry_sync
from CanonSync.src.core.syncdownload_processor import (
    process_syncdownloads,
)
...
def main():
    ...
    if len(sys.argv) < 2:
        show_message(
            'Uso: python CanonSync.src.py <caminho_destino> [dry-run]',
            'e',
        )
        return
